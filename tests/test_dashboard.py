...
class TestAgentContextHelpers:
    ...
    @pytest.mark.asyncio
    async def test_resolve_issue_from_pr_skips_pr_to_find_issue(self, tmp_path: Path) -> None:
        """_resolve_issue_from_pr skips PR references to find the real issue."""
        from unittest.mock import AsyncMock, MagicMock, patch
        ...
        mock_result = MagicMock()
        mock_result.success = True
        mock_result.stdout = "Closes #200 and Fixes #42"

        async def fake_is_issue(number, project_dir):
            return number == "42"
